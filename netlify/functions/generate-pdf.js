...
// Ubicación: netlify/functions/generate-pdf.js
...
// Busca la imagen en dist/ o public/ del servidor
function resolveImage(imgPath) {
  ...
  for (const base of bases) {
    const full = path.join(base, imgPath);
  ...
  return new Promise((resolve, reject) => {
    ...
    if (heroFile) {
      try {
        ...
        doc.image(heroFile, M, y, { width: CW, height: heroH });
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (gSrc) {
        try {
          ...
          doc.image(gSrc, gx, y, { width: gW, height: gH });
          doc.restore();
          continue;
        } catch {}
      }
